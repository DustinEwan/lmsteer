...
def _build_module_tree_recursive(module_to_inspect, parent_module_node: ModuleNode):
    for name_suffix, child_module in module_to_inspect.named_children():
        child_node = ModuleNode(name=name_suffix, module=child_module, parent_node=parent_module_node)
...
def build_module_tree(model: AutoModel) -> ModuleNode:
    ...
    root_internal_node = ModuleNode(name=model_class_name, module=model, parent_node=None)
    ...
    try:
        ...
        if tokenizer.pad_token is None:
            if tokenizer.eos_token is not None:
                ...
                console.print(f"Tokenizer for [bold cyan]{model_name}[/bold cyan] did not have a pad_token. Set to [bold green]eos_token[/bold green].")
            elif tokenizer.unk_token is not None:
                tokenizer.pad_token = tokenizer.unk_token
                console.print(f"Tokenizer for [bold cyan]{model_name}[/bold cyan] did not have a pad_token. Set to [bold green]unk_token[/bold green].")
            else:
                ...
                console.print(f"Tokenizer for [bold cyan]{model_name}[/bold cyan] did not have a pad_token. Added a new pad_token [bold green]'[PAD]'[/bold green].")
